...
void Compressor::compress(FILE* file, const std::vector<std::pair<size_t, long>>& poses) {
    if (poses.size() == 0) {
        throw std::exception("poses array is empty");
    }

    size_t last_element = poses[0].first;

    write_pos(file, last_element);

    for (size_t i = 1; i < poses.size(); i++) {
        if (poses[i].first <= last_element) {
            throw std::exception("poses array is not sorted");
        }

        size_t dif = poses[i].first - last_element;
        write_pos(file, dif);

        last_element = poses[i].first;
    }
}
